...
function TypewriterText({ text }: { text: string }) {
  ...
  useEffect(() => {
    ...
    if (!isDeleting && displayed.length < upperText.length) {
      timeout = setTimeout(() => {
        setDisplayed(upperText.slice(0, displayed.length + 1));
      }, 120);
    } else if (!isDeleting && displayed.length === upperText.length) {
      timeout = setTimeout(() => {
        setIsDeleting(true);
      }, 2200);
  ...
  }, [displayed, isDeleting, upperText]);
...
<<<<<<< HEAD
          ...
          <motion.div
            ...
            className="mt-2 text-xl font-bold uppercase tracking-widest text-white/90 sm:text-2xl"
          >
